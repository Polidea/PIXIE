...
int RestClient::readResponse(String* response) {
  ...
  //unsigned long unavailabilityCounter = 0;
  unsigned long timoutCounter = millis();
  
  while (client.connected() && (millis() - timoutCounter < 3000)) {
      ...
      if(httpBody){
        ...
        if (c=='}' && inJson){
            ...
            if (closingBrackets == openingBrackets){
                HTTP_DEBUG_PRINT("HTTP: return readResponseJson\n");
                return code;
            }
        }
      }
  ...
  if (millis() - timoutCounter >= 3000) {
  //if (unavailabilityCounter >= 1000) {
      HTTP_DEBUG_PRINT("HTTP: Strange timeout :( \n");
  }
